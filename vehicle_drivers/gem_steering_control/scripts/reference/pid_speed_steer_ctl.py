...
class PID_controller:
    ...
    def __init__(self, p=0.0, i=0.0, d=0.0, wg=20.0, avl=2.5, max_steering_angle=8.0, 
	...
	print(data)
        ...
        if data.data == 100 and emergency_stop_flag == False:
            print('DECISION MAKING DECLARED SAFE: ACCELERATING')
            accel_flag = True
            brake_cmd.f64_cmd = 0.0
            brake_pub.publish(brake_cmd)
        elif data.data == 100 and emergency_stop_flag == True and time.time()-emergency_stop_time >= 2:
            print('DECISION MAKING DECLARED SAFE: ACCELERATING')
            accel_flag = True
            brake_cmd.f64_cmd = 0.0
            brake_pub.publish(brake_cmd)
...
if __name__ == '__main__':
   ...
   # pid_controller = PID_controller(p=1.6, i=0, d=0.7, sp_p=1.35, sp_d=0.25, sp_i=0.08, desired_speed=0.3) #params for straight line
    pid_controller = PID_controller(p=6.1, i=0, d=2.7, sp_p=1.30, sp_d=0.02, sp_i=0.40, desired_speed=0.05) #params for curve
    ...
    rospy.Subscriber("/gem_decision", Char, pid_controller.decision_making_input)
