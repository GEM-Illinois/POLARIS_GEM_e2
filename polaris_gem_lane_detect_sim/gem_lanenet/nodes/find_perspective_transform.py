...
class TransformFinder:
    ...
    def __init__(self, tf_buffer: tf2_ros.Buffer, pub_annotated_img: rospy.Publisher):
        ...
        print(*self.MARKER_POS_LIST, sep='\n')
    ...
    def img_callback(self, img_msg: Image) -> None:
        ...
        try:
            ...
            for (u, v) in src_pts:
                cv2.circle(rectified_cv_img, (int(u), int(v)), radius=5, color=(0, 255, 0), thickness=-1)
            for (u, v) in dst_pts:
                cv2.circle(warped_cv_img, (int(u), int(v)), radius=5, color=(0, 255, 0), thickness=-1)

            # cropped margin to aligned the size
            src_h, src_w = rectified_cv_img.shape[0:2]
            dst_h, dst_w = warped_cv_img.shape[0:2]
            new_h, new_w = (dst_h - src_h) // 2, (dst_w - src_w) // 2
            warped_cv_img = warped_cv_img[new_h:new_h+src_h, new_w:new_w+src_w]

            annotated_img = np.vstack((rectified_cv_img, warped_cv_img))
